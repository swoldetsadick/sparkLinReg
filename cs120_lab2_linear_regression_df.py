# Databricks notebook source exported at Sun, 24 Jul 2016 23:18:20 UTC
# MAGIC %md
...
def parse_points(df):
    ...
    """
    return df.select(sql_functions.split(df.value,',').alias('value')).map(lambda x:LabeledPoint(x[0][0], x[0][1:])).toDF()
...
min_year = content_stats.map(lambda (x, y): x).collect()[0]
max_year = content_stats.map(lambda (x, y): y).collect()[0]
...
# TODO: Replace <FILL IN> with appropriate code
parsed_data_df = parsed_points_df.select('features', (parsed_points_df.label - min_year).alias('label'))#.withColumnRenamed('label - 1922.0', 'label')
...
average_train_year = (parsed_train_data_df
                        .selectExpr("avg(label)").withColumnRenamed('avg(label)', 'label').first())
...
# TODO: Replace <FILL IN> with appropriate code
preds_and_labels_train = parsed_train_data_df.map(lambda x : (average_train_year[0], x.label))
...
preds_and_labels_val = parsed_val_data_df.map(lambda x : (average_train_year[0], x.label))
...
preds_and_labels_test = parsed_test_data_df.map(lambda x : (average_train_year[0], x.label))
...
predictions = np.asarray(parsed_val_data_df
                         .rdd
                         .map(lambda lp: average_train_year[0])
                         .collect())
error = np.asarray(parsed_val_data_df
                   .rdd
                   .map(lambda lp: (lp.label, average_train_year[0]))
                   .map(lambda (l, p): squared_error(l, p))
                   .collect())
norm = Normalize()
clrs = cmap(np.asarray(norm(error)))[:,0:3]
...
def two_way_interactions(lp):
    ...
    return LabeledPoint(lp.label, liste)
